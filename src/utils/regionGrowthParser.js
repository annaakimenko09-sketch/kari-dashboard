...
function readRegionValues(ws, r, regions) {
  const values = {};
  for (const reg of regions) {
    const cell = ws[XLSX.utils.encode_cell({ r, c: reg.col })];
...
function parseRegionGrowthSheet(ws) {
  ...
  for (let r = SUMMARY_START_ROW; r <= SUMMARY_END_ROW; r++) {
    ...
    const values = readRegionValues(ws, r, regions);
  ...
  // Детальный список артикулов (с строки 48)
  const details = [];
